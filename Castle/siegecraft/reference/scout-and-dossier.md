...
The engine is only as good as its sources. Spin up a **background agent** (or parallel subagents) for the reading so you can keep aiming and structuring in parallel. If this harness has no background tool, do the research yourself first, write the dossier, then design â€” do not skip the artifact.
...
A vendor's speed claim is a claim by the vendor until you reproduce it. A README is marketing next to the code that ships. A tutorial, a summary site, a blog post, an AI answer, a wiki: **signposts** â€” use them to find the primary, never as the citation.
...
- **Read the reference implementation's source, not just its docs.** The behavior lives in the code and its tests. Where docs and code disagree, say which wins and why â€” usually the code, until the docs' version is confirmed by the maintainer or a test.
- **Unreachable primary â†’ `UNVERIFIED`, and de-loaded.** If the paper is paywalled, hunt the author's copy, the arXiv version, the conference site, the reference code. If the primary still cannot be reached this sitting, the claim is marked UNVERIFIED in the dossier and **cannot be load-bearing**: the engine is redesigned around the hole, or the sitting stops and says what is missing.
...
- **A finding that contradicts an ADR, the ticket's premise, or the slice Check is a stop** â€” aim again, or a cuecards sitting. Not a silent product change.

## The dossier

`docs/research/<engine-slug>.md` â€” the same species as oneslice's research files, heavier. Committed before anything that depends on it.

```markdown
# Dossier: <the question, in one sentence>

- **For engine:** docs/siegecraft/<slug>.md
- **Date:** YYYY-MM-DD Â· **Researched by:** <agent name>

## Sources consulted
| # | Source | Kind | What it owns | Link / path |
| --- | --- | --- | --- | --- |
| 1 | <RFC 9110 Â§5.1> | primary spec | <semantic of the field> | <url> |
| 2 | <Author, "Title", venue year, Â§4> | primary paper | <the guarantee> | <url or local copy path> |
| 3 | <org/repo @ commit, src/thing.ts L120-180> | reference code | <actual behavior> | <permalink> |

## Findings
### F1. <claim>
- **CITED** â€” <owner, exact anchor>: <what it actually says â€” quote or tight paraphrase, not a summary of a summary>
- **So what:** <consequence for the engine>

## Measured
<command, machine, date, input size â€” output pasted>
...
Findings are rewritten so the next reader â€” the spec, the builder, the next siegecraft sitting â€” can use them without re-opening the paper. Facts are the scout's job; choices are not.
