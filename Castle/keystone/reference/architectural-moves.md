...
## 2. Cut â€” find the axes of change
...
- The GUI doesn't matter to the business rules â†’ line between them.
- The database doesn't matter to the business rules â†’ line between them.
- The report formatter doesn't matter to the pricing policy â†’ line between them.
- Reporting doesn't matter to booking â†’ line between them, and a named integration pattern across it.

Do not cut where nothing moves. A boundary that has never been crossed in anger is cost with no return: interfaces to maintain, data to marshal, deployments to coordinate. Equally, do not skip a boundary where friction already exists â€” retrofitting one under load, even with good tests and refactoring discipline, is one of the most expensive things a team can attempt.
...
## 3. Map â€” contexts, and the relationship between each pair
...
The seven named patterns â€” shared kernel, customer/supplier, conformist, anti-corruption
layer, open host service, published language, separate ways â€” with when each fits, what it
...
## 4. Distill â€” find the core, and say it small
...
- **Core domain** â€” the part that is the reason this system exists, that would justify building rather than buying.
- **Supporting subdomains** â€” needed, not differentiating. Build them plainly; do not put your best people here.
- **Generic subdomains** â€” buy them, adopt them, or isolate them behind an interface. Leave no trace of your specialties in them.
- **Cohesive mechanisms** â€” when a part of the model is really a well-understood formalism or algorithm, partition it into its own lightweight framework behind an intention-revealing interface, and let the core express *what* rather than *how*.
...
- Nothing outside may hold a reference to anything inside except the root. Transient references to internal members may be passed out for use within a single operation only. A **value object** may be copied out freely â€” it is just a value, and what happens to it does not matter.
...
- **Too large** â†’ contention, lock scope, optimistic-concurrency failures under load, a transaction that spans concepts that do not actually need to be consistent together.
- **Too small** â†’ invariants that cannot be enforced transactionally at all, chatty cross-aggregate coordination, and a domain that has become a set of anemic property bags around a database.

Ask of every invariant: **does this have to be true at the end of every transaction, or is "within N seconds" acceptable?** The answer, per invariant, is the boundary. If the honest answer is "immediately" for two things you have separated, merge them. If it is "eventually" for two things you have merged, split them and name the mechanism â€” a domain event, a scheduled reconciliation, a batch â€” and state the time bound. An unstated time bound is an untested assumption that will surface as a production incident at 3am.
...
**Level = distance from the inputs and outputs.** The farther a policy is from both, the higher its level. The policies that manage I/O are the lowest in the system. In a good architecture, source dependencies are decoupled from data flow and coupled to level â€” which is why the arrow from a use case to a presenter points inward even though control flows outward. You get that by having the inner component define the interface and the outer component implement it.

The canonical rings, adapted to what this system actually is â€” the count is not the point, the direction is:
...
## 11. Record â€” the ADRs

Every structural decision that a future engineer might otherwise relitigate gets an ADR. That is the bar. Format, numbering, and lifecycle come from cuecards â€” `docs/adr/NNNN-slug.md`, append-only, superseded by a later ADR rather than edited.
...
## 12. Hold â€” review against the record
...
- Did it contradict an ADR? Then stop â€” that is a cuecards sitting, not a comment.
