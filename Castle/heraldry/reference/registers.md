# Design Registers and Reading the Room
...
Two registers in one product is two brands. A split is legitimate only when the design system (DESIGN.md) say so and the seam is obvious: a marketing site may be Cathedral while the product it sells is Keep. Everything else picks one.
